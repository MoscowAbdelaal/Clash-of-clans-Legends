...
public class Game {
    ...
    public void moveUp(int x, int y) throws Exception {
        if (x < 0 || x >= 10 || y < 0 || y >= 10) {
            throw new InvalidActionException("Player cannot move outside of bounds.");
        }

        if (!(map[x][y] instanceof UnitCell)) {
            throw new InvalidActionException("Player can only move units.");
        }
        ...
        if (!(map[newX][y] instanceof EmptyCell) && !(map[newX][y] instanceof ResourceCell)) {
            throw new InvalidActionException("Player can only move to Empty or Resource Cells.");
        }

        // Handle resource collection if moving to a ResourceCell
        if (map[newX][y] instanceof ResourceCell) {
            ResourceCell resourceCell = (ResourceCell) map[newX][y];
            if (resourceCell.getType() == ResourceType.GOLD) {
                currentPlayer.setGoldAmount(currentPlayer.getGoldAmount() + resourceCell.getAmount());
            } else if (resourceCell.getType() == ResourceType.MANPOWER) {
                currentPlayer.setManpowerAmount(currentPlayer.getManpowerAmount() + resourceCell.getAmount());
            }
        }
    ...
    public void moveLeft(int x, int y) throws Exception {
        ...
        // Handle resource collection if moving to a ResourceCell
        if (map[x][newY] instanceof ResourceCell) {
            ResourceCell resourceCell = (ResourceCell) map[x][newY];
            if (resourceCell.getType() == ResourceType.GOLD) {
                currentPlayer.setGoldAmount(currentPlayer.getGoldAmount() + resourceCell.getAmount());
            } else if (resourceCell.getType() == ResourceType.MANPOWER) {
                currentPlayer.setManpowerAmount(currentPlayer.getManpowerAmount() + resourceCell.getAmount());
    ...
    public void build(int x, int y, int tx, int ty, String building) throws Exception {
        ...
        if (!(map[x][y] instanceof UnitCell)) {
            throw new InvalidActionException("Player can only build using heroes.");
        }
        ...
        if (hero.getType() != HeroType.PACIFIST) {
            throw new InvalidActionException("Player can only build using pacifist heroes.");
        }

        int dx = Math.abs(x - tx);
        int dy = Math.abs(y - ty);
        if ((dx == 1 && dy == 1) || (dx == 1 && dy == 0) || (dx == 0 && dy == 1)) {
            // Allow orthogonally adjacent (dx=1,dy=0 or dx=0,dy=1) and diagonally adjacent (dx=1,dy=1)
        } else {
            throw new InvalidActionException("Player can only build on adjacent Cells.");
        }
    ...
    public void recruitUnit(int x, int y, String unit) throws Exception {
        ...
        if (!(building instanceof Barracks)) {
            throw new InvalidTargetException("Player can only recruit from building cells that contain barracks.");
        }
        ...
        if (spawnY >= 10) {
            throw new InvalidActionException("No space to spawn unit");
        }
    ...
    public void upgrade(int x, int y) throws Exception {
        ...
        if (map[x][y] instanceof UnitCell) {
            ...
            if (!(unit instanceof SupportUnit)) {
                throw new InvalidTargetException("Player can only target UnitCells that contain SupportUnits or BuildingCells for upgrades.");
            }
            ...
            throw new InvalidTargetException("Player can only target UnitCells or BuildingCells for upgrades.");
        }
    }
    ...
    public void useSpecial(int x, int y, int tx, int ty) throws Exception {
        ...
        if (!(map[x][y] instanceof UnitCell)) {
            throw new InvalidActionException("Player can only use special using heroes.");
        }
        ...
        if (!(targetUnit instanceof SupportUnit)) {
            throw new InvalidTargetException("Player can only use special actions targetting support units.");
        }
        ...
        if (distance > hero.getRange()) {
            throw new InvalidActionException("Player can only use special actions on targets within their range.");
        }
    ...
    public void attack(int x, int y, int tx, int ty) throws Exception {
        ...
        if (!(map[x][y] instanceof UnitCell) && !(map[x][y] instanceof BuildingCell)) {
            throw new InvalidActionException("Player can only attack using valid attacking units or towers.");
        }
        ...
        if (map[x][y] instanceof UnitCell) {
            ...
            if (attacker instanceof Hero) {
                ...
                if (hero.getType() != HeroType.AGGRESSOR) {
                    throw new InvalidActionException("Only Aggressor type heroes can attack.");
                }
            }
        ...
        if (distance > range) {
            throw new InvalidActionException("Units can only attack enemy units or buildings that are within range.");
        }
        ...
        if (attacker != null) {
            attacker.setActionsAvailable(0);
        } else if (buildingAttacker instanceof Tower) {
            ((Tower) buildingAttacker).setCanAttack(false);
        }
    }
